  ...
  return (
    <div className="flex justify-center items-center">
      <Card className=" rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300">
          ...
          <div className="flex items-center gap-4">
            <div className="flex gap-1">
              {Array.from({ length: props.product.rating }).map((i, index) => (
                <FaRegStar key={index} className="text-yellow-500 size-5" />
              ))}
            </div>
        ...
        <CardFooter>
          <button className="w-full cursor-pointer text-center py-2 font-semibold border-2 text-lg border-gray-800 rounded-full transition-all duration-300 hover:bg-gray-800 hover:text-white">
            Add To Cart
          </button>
        </CardFooter>
      </Card>
